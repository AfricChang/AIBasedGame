...
- `AudioService`
  ...
  - 成功/失败音效
  - 静音与音量
  - 优先使用 `Web Audio API` 或轻量参数化音效，避免初期引入较大的音频资源包
...
本项目当前采用原生 Canvas 渲染，表现增强优先保持轻量、低依赖和移动端稳定。

#### 可参考资源

1. `Kenney Tiny Dungeon` / `Kenney Modular Dungeon Kit`
   - 授权：`Creative Commons CC0`
   - 用途：地牢、木质地板、墙体和小型道具的视觉风格参考。
2. `OpenGameArt Top Down Dungeon Pack` / `Dungeon tileset`
   - 授权：需逐项确认，优先选择 `CC0` 或明确允许商用的素材。
   - 用途：俯视角地板、墙体、机关图标参考。
3. `drewsilcock/maze.js`
   - 授权：`MIT`
   - 用途：轻量 HTML5 Canvas 迷宫渲染参考。
4. `dahlskebank/magic-maze-web`
   - 授权：`MIT`
   - 用途：低技术栈迷宫游戏表现参考。
5. `KilledByAPixel/ZzFX`
   - 授权：`MIT`
   - 用途：极小体积 JavaScript 参数化音效参考。
6. `chr15m/jsfxr`
   - 授权：`Unlicense`
   - 用途：复古短音效参数设计参考。
...
1. 程序化木质地板
   - 在 Canvas 中按格子绘制木板底色、细木纹和轻微明暗变化。
   - 不引入图片文件，避免包体增长和素材授权风险。
   - 纹理对比必须克制，不能干扰墙体、单向门、钥匙和出口识别。
2. 轻量 Web Audio 音效
   - 使用 `Web Audio API` 生成移动、撞墙、拾取钥匙、开门、传送、信标、通关和失败音效。
   - 不播放背景音乐，优先做短促反馈音，避免连续移动时产生噪声疲劳。
   - 保留音效开关，为移动端浏览器自动播放限制和用户偏好留出口。
3. 外部素材包评估
   - 若后续需要更强美术表现，再评估引入 `CC0` 图片素材。
   - 引入前需记录来源、授权、压缩后体积和移动端渲染成本。
